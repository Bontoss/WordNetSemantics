...
int solve(int node, int turn) // O(N + M)
{
...
pair<int, int> solveDP(string &x, string &y) // O(N + M)
{
...
int main()
{
    ...
    {                                                    // O(N * (N + M))
        pair<int, int> ans = solveDP(u.first, u.second); // O(N + M)
        cout << ans.first << ',';
        for (auto u : IDToNouns[ans.second]) // O(N)
            cout << u << ' ';
        cout << '\n';
    }
